...
void DashboardPage::setupUI()
{
    ...
    QLabel *headerSubtitle =
        new QLabel("Welcome back! Here's your café overview.");
    ...
    // Remove shadow from the table itself (it's in the card now)
...
void DashboardPage::setupRecentOrdersTable()
{
    ...
    m_recentOrdersTable->setSelectionBehavior(
    ...
    // Row height
    m_recentOrdersTable->verticalHeader()
        ->setDefaultSectionSize(52);

    // ==============================
    // Shadow
    // ==============================
    QGraphicsDropShadowEffect *shadow =
        new QGraphicsDropShadowEffect(m_recentOrdersTable);
...
void DashboardPage::populateStats()
{
    m_revenueCard->setValue(
        QString("$%L1")
            .arg(m_dataModel->totalRevenue(), 0, 'f', 0));

    m_revenueCard->setSubtitle(
        QString("+%1% from last month")
            .arg(m_dataModel->revenueGrowth(), 0, 'f', 1));

    m_ordersCard->setValue(
        QString("%L1")
            .arg(m_dataModel->totalOrders()));

    m_ordersCard->setSubtitle(
        QString("+%1% from last month")
            .arg(m_dataModel->ordersGrowth(), 0, 'f', 1));

    m_employeesCard->setValue(
        QString::number(m_dataModel->activeEmployees()));

    m_employeesCard->setSubtitle(
        QString("%1 working right now")
            .arg(m_dataModel->workingNow()));

    m_topProductCard->setValue(
        m_dataModel->topSellingProduct());

    m_topProductCard->setSubtitle(
        QString("%1 orders this month")
            .arg(m_dataModel->topSellingCount()));
}
...
void DashboardPage::populateCharts()
{
    ...
    m_lineChart->setLabels({
        "Mon", "Tue", "Wed",
        "Thu", "Fri", "Sat", "Sun"
    });

    QList<int> orderData = m_dataModel->weeklyOrders();

    m_barChart->setData(orderData);

    m_barChart->setLabels({
        "Mon", "Tue", "Wed",
        "Thu", "Fri", "Sat", "Sun"
    });
}
...
void DashboardPage::populateOrders()
{
    ...
    for (int i = 0; i < orders.size(); i++) {
        ...
        // ⭐ تحديد العرض بناءً على النص (badge صغير)
        QFont badgeFont("Segoe UI", 9);
        badgeFont.setBold(true);
        statusLabel->setFont(badgeFont);

        QFontMetrics fm(badgeFont);
        int textWidth = fm.horizontalAdvance(order.status) + 24; // padding
        ...
        statusLayout->addStretch();  // ⭐ يخلي الـ badge على الشمال

        m_recentOrdersTable->setCellWidget(i, 5, statusContainer);
    }
}
void DashboardPage::styleStatusLabel(
    QLabel *label,
