...
void DatabaseManager::closeConnection()
{
    ...
    // امسح المرجع قبل ما تشيل الـ Connection
    m_db = QSqlDatabase();

    // شيل الـ Connection من Qt
...
bool DatabaseManager::updateEmployee(int id, const QString &name, bool isActive, const QString &role)
{
    if (!isConnected()) { setError("Database not connected"); return false; }

    QSqlQuery query(m_db);
    query.prepare("UPDATE Employees SET EmployeeName=?, IsActive=?, Role=? WHERE EmployeeID=?");
    query.addBindValue(name);
    query.addBindValue(isActive ? 1 : 0);
...
bool DatabaseManager::deleteEmployee(int id)
{
    ...
    QSqlQuery query(m_db);
    query.prepare("UPDATE Orders SET EmployeeID = NULL WHERE EmployeeID = ?");
    query.addBindValue(id);
    query.exec();
